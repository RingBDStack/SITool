...
class FastMergeSEM:
    ...
    def sum_up_multi_edge(edges, *weights, operation_ptrs=None):
        if operation_ptrs is not None:
            ...
            trans = FastMergeSEM.get_edge_transform(em)
            redu = FastMergeSEM.reduction_edge(em, trans, *wm)
            cnt = redu[0].shape[0]
            ...
            trans = FastMergeSEM.get_edge_transform(edges)
            return FastMergeSEM.reduction_edge(edges, trans, *weights)
    ...
    def process_fast(self, p=1.0,
        ...
        assert 0.0 <= p <= 1.0
        edges, trans_prob = self.partition.graph.edges
        ...
        com0 = torch.arange(self.partition.graph.num_vertices, device=self.partition.graph.device)
        transs = []
        merge_all = False
        if loop0 is None:
            # noinspection PyUnresolvedReferences
            loop0 = bool((edges[:, 0] == edges[:, 1]).any())
        ...
        current_num_vertices = self.partition.graph.num_vertices

        t = time.time()

        if re_compute is not True:
            trans = self.partition.node_id
            if adj_cover is None:
                cache = False
                edges = trans[edges]
                edges, trans_prob = FastMergeSEM.sum_up_multi_edge(edges, trans_prob)
                ...
                cache = False
                edges = trans[edges]
                edges, trans_prob, adj_cover = FastMergeSEM.sum_up_multi_edge(edges, trans_prob, adj_cover)
                current_num_vertices = edges.max() + 1
            transs.append(trans)
        ...
        while operated_cnt > ter or not merge_all:
            ...
            # t = time.time()
            # print(self.partition.graph.num_vertices)
            if not cache:
                v1 = silearn.scatter_sum(trans_prob, edge_t, clip_length= mx + 1).reshape(-1)
                # print(self.partition.graph.stationary_dist)
                vst = v1[edges]
            ...
            # print(operated_cnt)
            merge = op
            # rand_idx = torch.randint(0,  2**31,(1, self.partition.graph.num_vertices), device=self.device)[0]
            ...
            trans = torch.arange(edges.max() + 1, device=self.partition.graph.device)

            trans[ids] = trans[idt]
            # if operated_cnt < 10:
            #     altered = torch.zeros(self.partition.graph.num_vertices, device=self.partition.graph.device, dtype=torch.bool)
            #     altered[ids] = True
            #     altered[idt] = True
            ...
            # var = #trans != torch.arange(self.partition.graph.num_vertices, device=self.partition.graph.device)
            ...
            # t = time.time()
            # failed = torch.logical_or(trans == torch.arange(self.partition.graph.num_vertices) , altered) == 0
            # print(ids)
            # print(idt)
            # print(torch.nonzero(failed))

            # print(torch.nonzero(altered))
            # print(torch.nonzero(trans != torch.arange(self.partition.graph.num_vertices)))
            ...
            if self.partition.graph.num_vertices - operated_cnt == min_com:
                break
        ...
        if len(transs) != 0:
            ...
            self.partition.node_id = trans
        else:
            self.partition.node_id = com0

    def iterative_merge(self, verbose = False, min_com = 1,
                        max_iteration = 30, tau = 0.1, sample_ratio = 0.5, p = 0.5, m_scale = -1):
        prob_e = torch.ones(self.partition.graph.num_edges, device=self.partition.graph.device)
        edges, _ = self.partition.graph.edges
        for i in range(max_iteration):

            rand = torch.rand(self.partition.graph.num_edges, device=self.partition.graph.device) * prob_e
            bound = torch.msort(rand)[int((prob_e.shape[0] - 1) * sample_ratio)]

            cover_adj = rand >= bound
            self.process_fast(adj_cover = cover_adj,min_com = min_com, p = p, m_scale= m_scale)
            self.process_fast(min_com = min_com, re_compute=False, p = p, m_scale= m_scale)
            c = torch.logical_not(cover_adj)
            operated = self.partition.node_id[edges[:, 0]] == self.partition.node_id[edges[:, 1]]
            # prob_e[torch.logical_and(cover_adj,torch.logical_not(operated))] *= 1 - tau
            prob_e[torch.logical_and(c,operated)] /= 1 - tau
            print(prob_e)

            print(self.partition.structural_entropy(reduction="sum"))

    # By Hujin

    def process_c(self, erase_loop=True, adj_cover=None, target_cluster = 0, m_scale =0):
        ...
        node_cnt = self.partition.graph.num_vertices
        ...
        self.partition.node_id = torch.LongTensor(result.astype(np.int64)).to(self.partition.graph.device)
