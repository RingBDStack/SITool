...
class ImageSeg(object):
    # if self.verbose: print algorithm information
    verbose = True
    # use_gpu: compute on cuda if available
    use_gpu = True
    # buffer of the entropy map
    # [1Dim SE, 2Dim SE, Decoding Information, Entropy Rate]
    dec_map = None

    """
    Algorithm parameters
    """
    # threshold generating full graph
    thresh_full_g = 500
    # weight of edges inserted for enforcing connectivity
    lambda_connectivity_edges = 0.1
    t = .5
    # dynamic t, 1.0 for fixed t
    t_decay = 1.0
    # KNN parameters
    k, r, k2, r2 = 25, 9, 48, 11
    # regional distance parameters
    lambda_t_regional = 1.5
    lambda_weight_regional = 1.0
    scaling_bound = math.inf

    def __init__(self, verbose=True, use_gpu=True):
    ...
    def process(self, img_input, target_num=100, force_connectivity=True, use_lab=True,
        ...
        if fast_comp:
            dd = torch.abs((e0 - e1))
            aaa = (dd <= 1) + (dd == imgW) + (dd == imgW - 1) + (dd == imgW + 1)
            si.process_fast(adj_cover=aaa)
        else:
            si.target_cluster = 10
            si.global_var = 0
            dd = torch.abs((e0 - e1))
            aaa = (dd <= 1) + (dd == imgW) + (dd == imgW - 1) + (dd == imgW + 1)
            si.process(adj_cover=aaa)
        si.graph = None
        del w, g, e0, e1, es, et
        ...
        norm = float(torch.mean(abs(w)))
        e0, e1, w = self.generate_weights(es, et, t, w)
        si.graph = cogdl.data.Graph(edge_index=(e0, e1), edge_weight=w)
        ...
        while (com_sz_smmoth > target_num):
            ...
            cnt2 = cluster_cnt(com2)
            com_sz_smmoth = (cnt2 > 10).sum()

            si = sii
            if intercode and pos_inter_node > 0:
                cntcom = cluster_cnt(sii.community_result)
